...
empty envelope) is skipped via the injected output sink (``self._out.info``),
and only when EVERY fetch fails does the service raise :class:`NoSkewData`
...
class SkewService(BaseService):
    """Layer-2 service for the ``skew`` command (all five modes)."""
    ...
    def _fetch_for_report(
        ...
        """Like :meth:`_fetch_single_chain` but converts failures into a
        skip notice (``self._out.info``) + ``None``. Used by the term / cross
        modes where we want to render whatever chains succeeded instead of
    ...
    def get_skew_l1(
        ...
        *,
        ...
        with self._authed_client() as client:
            envelope = self._fetch_single_chain(client, symbol, expiry, strikes)
    ...
    def get_skew_term(
        ...
        *,
        ...
        Per-expiry fetch failures are skipped (``self._out.info``); only when
        EVERY expiry fails does this raise :class:`NoSkewData`.
        """
        envelopes: list[dict[str, Any]] = []
        with self._authed_client() as client:
            for exp in expiries:
                env = self._fetch_for_report(client, symbol, exp, strikes)
                if env is not None:
    ...
    def get_skew_dtes(
        ...
        *,
        ...
        Per-expiry fetch failures are skipped (``self._out.info``).
        """
        with self._authed_client() as client:
            available = self._discover_expiries(
                client, symbol, max_dte=max(target_dtes)
            )
            if not available:
                raise NoSkewData(
    ...
    def get_skew_cross(
        ...
        *,
        ...
        Per-symbol fetch failures are skipped (``self._out.info``); only when
        EVERY symbol fails does this raise :class:`NoSkewData`.
        """
        envelopes: list[dict[str, Any]] = []
        with self._authed_client() as client:
            for sym in symbols:
                env = self._fetch_for_report(client, sym, expiry, strikes)
                if env is not None:
                    envelopes.append(env)
        if not envelopes:
            raise NoSkewData(
    ...
    def get_skew_cross_dtes(
        ...
        *,
        ...
        Per-symbol discovery-empty or fetch failures are skipped
        (``self._out.info``); only when EVERY symbol yields nothing does this
        raise :class:`NoSkewData`.
        """
        envelopes: list[dict[str, Any]] = []
        with self._authed_client() as client:
            for sym in symbols:
                try:
                    available = self._discover_expiries(
                        client, sym, max_dte=target_dte + 30
                    )
                except (ApiError, SessionExpired) as e:
                ...
                env = self._fetch_for_report(client, sym, exp, strikes)
                if env is not None:
                    envelopes.append(env)
        if not envelopes:
            raise NoSkewData(
