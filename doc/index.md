...
| [`mcp`](mcp.md) | Run `schwab_cli` as an MCP server so agents can call Schwab tools directly. Streamable HTTP transport (single `/mcp`); status / log / install / logout subcommands. |
| [`server`](server.md) | Auth-maintenance daemon that keeps the 7-day refresh token alive. `--enable-mcp` adds the Streamable HTTP MCP server; `--enable-rest` adds an unauthenticated REST PoC. |
